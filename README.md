...
#### Поднято две машины - веб-сервер NGINX (web) и лог-сервер (log). Логи собираются с помощью rsyslog.
...
```
- принудительно впесем в журнал запись со статусом **crit**
...
- проверка журнала аудита на веб-сервере
