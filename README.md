...
```
- Переименовываем VG VolGroup00 в OtusRoot
```
[root@lvm ~]# vgrename VolGroup00 OtusRoot
  Volume group "VolGroup00" successfully renamed to "OtusRoot"
```
- Правлю конфигурационные файлы, меняю название VG
```
# /etc/fstab
...
```
- Скачиваю с помощью wget прилагаемые скрипты
```
[root@lvm 01test]# pwd
...
- Проверяю, загрузился ли модуль test
```
[root@lvm 01test]# lsinitrd -m /boot/initramfs-$(uname -r).img | grep test
test
```
- Из GUI Virtualbox перезагружаю VM, при появлении меню выбора ядра нажимаю e(edit) и попадаю в окно изменения параметров загрузки.
- Вручную удаляю опции rghb и quiet, загружаюсь, нажав Ctrl+X
- Вуаля, пришел пингвин :)
